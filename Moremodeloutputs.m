function [Incidence, Num_lat, Num_PCR, Num_conf, School_incidence, Num_lat_t] = Moremodeloutputs(history)
...
Weeks = 8;
YearSize = size(history.Infection,2);
YearGroup = size(history.Infection,1);
...
%Number of lateral flows through time
Num_lat_t = zeros(1,Weeks*7);
Num_lat_t(Schooldaysandinitial) = (sum(history.Test_days(:, Schooldaysandinitial).*Not_isolating));
